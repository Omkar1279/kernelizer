...
/// Translates a vague prompt into strict KERNEL parameters using `claude --bare`.
/// Receives both explicit file content and the repo map so Haiku can write
/// codebase-specific constraints rather than generic ones.
pub async fn translate_with_cli(
    prompt: &str,
    file_context: &str,
    repo_map: &str,
) -> Option<KernelParams> {
    ...
    if !repo_ctx.is_empty() {
        parts.push(format!("Repo Map (all function/type signatures in this codebase):\n{repo_ctx}"));
    }
    if !file_ctx.is_empty() {
        parts.push(format!("File Context (explicitly provided files):\n{file_ctx}"));
    }
...
fn cap_at_char_boundary(s: &str, cap: usize) -> &str {
    if s.len() <= cap {
        return s;
    }
    let mut end = cap;
...
/// explicit files, STDIN, and the tree-sitter repo map.
#[must_use]
pub fn compile_kernel(
    ...
    let has_repo_map = !repo_map.is_empty();

    let has_any_context = has_explicit_files || has_repo_map || !stdin_context.is_empty();
    ...
    if has_repo_map {
        output.push_str("## Repo Map (function & type signatures):\n```\n");
    ...
    let tool_usage_constraint = match (has_explicit_files, has_repo_map) {
        ...
        (false, true) => "- Tool Usage: ## Repo Map lists all known symbols. Use it to locate files before opening them — do not search for symbols already listed above.\n",
    ...
    let pattern_constraint = if has_repo_map {
        if follow_pattern {
            Some("- Pattern Adherence (STRICT): The user explicitly requested the project's coding style. You MUST implement using patterns consistent with ## Repo Map signatures — no deviations.\n")
        } else {
            Some("- Pattern Reference (ADVISORY): ## Repo Map shows the project's existing structure. Use it as context but improve on it if a better, idiomatic solution exists.\n")
        }
    } else {
        None
    };
